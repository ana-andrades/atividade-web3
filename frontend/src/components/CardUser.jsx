...
export default function CardUser({ id, avatar, name, email }) {
    ...
    const handleDelete = async () => {
        const response = await fetch(`http://127.0.0.1:3333/user/${id}`, {
            method: "DELETE"
        })
...
const styles = {
    container: {
        ...
        borderRadius: 8,
        position: "relative", // ESSENCIAL: para os ícones ficarem presos aqui dentro
        backgroundColor: "#fff",
        marginBottom: 10
    },
    image: {
    ...
    icons: {
        cursor: "pointer",
        color: "#2c2c2c",
        fontSize: 15, // Garante que o tamanho seja aplicado
    },
    form: {
        display: "flex",
